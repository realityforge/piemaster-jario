...
public class PlayerHandlingSystem extends EmptyHandlingSystem
{
	...
	{
		// Invulnerable player ignores collision
		Invulnerable invuln = invulnMapper.get(player);
		if(invuln != null)
		{
			return;
		}
...
//				healthMapper.get(player).setInvulnerable(false);
	...
	protected void handleItemCollision(Entity player, Entity item, EdgeType edge)
	{
		...
		case STAR:
			InvulnerabilityHandler.setTemporaryInvulnerability(world, player, 10000, true, true);
			SoundSystem.pushSound(SoundSystem.POWERUP_SOUND, player);
			break;
	...
	{
		if (invulnMapper.get(player) == null)
		{
			Health health = healthMapper.get(player);
			if (health.getHealth() > 1)
			{
			...
			else
			{
				...
				InvulnerabilityHandler.setTemporaryInvulnerability(world, player, RECOVERY_TIME,
						true);
